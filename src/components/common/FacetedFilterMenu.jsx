...
export default function FacetedFilterMenu({
    ...
    // Calculate position based on anchor element - prevent overflow
    useEffect(() => {
        if (isOpen && anchorRef?.current) {
            const rect = anchorRef.current.getBoundingClientRect()
            const viewportHeight = window.innerHeight
            const menuMaxHeight = Math.min(500, viewportHeight - 100) // Max menu height
            const spaceBelow = viewportHeight - rect.bottom - 16 // Space below button
            const spaceAbove = rect.top - 16 // Space above button

            let top, maxHeight

            if (spaceBelow >= menuMaxHeight || spaceBelow >= spaceAbove) {
                // Open below - enough space or more space below than above
                top = rect.bottom + 4
                maxHeight = Math.min(menuMaxHeight, spaceBelow)
            } else {
                // Open above - more space above
            ...
            setMenuPosition({
                top: Math.max(8, top),
                left: Math.max(8, Math.min(rect.left, window.innerWidth - 360)), // Prevent right overflow
                maxHeight: maxHeight
            })
        }
    }, [isOpen, anchorRef])
    ...
    return createPortal(
        <div
            ...
            style={{
                ...
                maxHeight: menuPosition.maxHeight || 500,
                zIndex: 9999
            }}
        >
            {/* Header */}
