    ...
    const IssueRow = ({ issue, isDragging = false }) => {
        ...
        return (
            ...
            >
                ...
                {/* Parent Epic - respects visibility setting */}
                {cardFieldVisibility.parent && issue.parentId && (() => {
                    const parentEpic = issues.find(i => i.id === issue.parentId && i.type === 'epic')
                    return parentEpic ? (
                        <span className="issue-parent-badge" title={parentEpic.summary}>
                            <span className="epic-color" style={{ background: 'var(--epic)' }} />
                            {parentEpic.summary.length > 15
                                ? parentEpic.summary.substring(0, 15) + '...'
                                : parentEpic.summary}
                        </span>
                    ) : null
                })()}
                ...
                <div className="epic-list">
                    <button
                        ...
                        onDrop={async (e) => {
                            ...
                            const issueId = e.dataTransfer.getData('issueId')
                            if (issueId) {
                                await updateIssue(issueId, { parentId: null })
                            }
                        }}
                    >
                            ...
                            <div key={epic.id} className="epic-item-wrapper" title={epic.summary}>
                                ...
                                <button
                                    ...
                                    onDrop={async (e) => {
                                        ...
                                        const issueId = e.dataTransfer.getData('issueId')
                                        if (issueId) {
                                            await updateIssue(issueId, { parentId: epic.id })
                                        }
                                    }}
                                >
                                    <span className="epic-color" style={{ background: 'var(--epic)' }} />
