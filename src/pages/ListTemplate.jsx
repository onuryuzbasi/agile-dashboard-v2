...
function DatePicker({ value, onChange, onClose, position }) {
    ...
    return (
        <div className="jira-date-picker" onClick={e => e.stopPropagation()} style={pickerStyle}>
...
export default function ListTemplate() {
    ...
    // All issues are top-level (no child grouping under epics)
    const isTopLevelIssue = (issue) => {
        // Epics are top-level
        if (issue.type === 'epic') return true
        // All other issues are also top-level (independent in their own sprint)
        return true
    }
                                            ...
                                            <React.Fragment key={issue.id}>
                                                ...
                                                >
                                                    <div className="pinned-columns">
                                                        ...
                                                        <div className="cell key key-stacked" style={{ width: keyWidth, minWidth: keyWidth }}>
                                                            {/* Parent key on top (if exists) */}
                                                            {parentIssue && (
                                                                <span
                                                            ...
                                                            {/* Issue key (with indent icon if has parent) */}
                                                            <span className="key-row">
                                                                {parentIssue && <span className="key-indent">↳</span>}
                                                                <span
                                                                    className={`issue-key-link ${isEpic ? 'epic-key' : ''}`}
                                                                    onClick={(e) => {
                                                                        e.stopPropagation()
