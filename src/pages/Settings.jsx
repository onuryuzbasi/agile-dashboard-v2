...
export default function Settings() {
    ...
    // Invite management state (admin only)
    ...
    const fetchInvitedEmails = useCallback(async () => {
        const { data, error } = await supabase
            .from('invited_emails')
            .select('*')
        if (!error && data) setInvitedEmails(data)
    }, [])

    useEffect(() => {
        if (isAdmin) fetchInvitedEmails()
    }, [isAdmin, fetchInvitedEmails])
    ...
    const handleAddInvite = async () => {
        ...
        const exists = invitedEmails.some(e => e.email === email)
        if (exists) {
            setInviteError('Bu e-posta zaten davetli.')
        ...
        const { error } = await supabase
            .from('invited_emails')
            .insert({ email, role: newInviteRole })
        if (error) {
            setInviteError('Hata: ' + error.message)
        } else {
            setInviteSuccess(`${email} başarıyla davet edildi!`)
            setNewInviteEmail('')
    ...
    const handleRemoveInvite = async (id, email) => {
        showConfirmModal({
            title: `"${email}" davetini kaldır?`,
            message: 'Bu kullanıcı artık giriş yapamayacak.',
            variant: 'danger',
            confirmText: 'Kaldır',
            onConfirm: async () => {
    ...
    const handleUpdateRole = async (id, newRole) => {
        await supabase.from('invited_emails').update({ role: newRole }).eq('id', id)
        await fetchInvitedEmails()
    }
            ...
            {activeTab === 'invites' && isAdmin && (
                <>
                    <SettingSection
                        ...
                        description="Davet edilen e-postalar Google Sign-In ile giriş yapabilir"
                    >
                        <div className="flex flex-col gap-4">
                            {/* Add invite form */}
                            ...
                            {inviteError && (
                                <div style={{ padding: '8px 12px', borderRadius: 'var(--radius-md)', background: 'var(--danger-light)', color: 'var(--danger)', fontSize: '13px' }}>
                                    <AlertCircle size={14} style={{ display: 'inline', marginRight: '6px', verticalAlign: '-2px' }} />
                                    {inviteError}
                                </div>
                            )}
                            {inviteSuccess && (
                                <div style={{ padding: '8px 12px', borderRadius: 'var(--radius-md)', background: 'var(--success-light)', color: 'var(--success)', fontSize: '13px' }}>
                            ...
                            <div style={{ marginTop: '8px' }}>
                                <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '8px' }}>
                                    Davetli Kullanıcılar ({invitedEmails.length})
                                </div>
                                        ...
                                        >
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => handleUpdateRole(invite.id, e.target.value)}
                                                    style={{
                                                        padding: '2px 8px',
                                                        borderRadius: '999px',
                                                        fontSize: '11px',
                                                        fontWeight: 600,
                                                        background: invite.role === 'admin' ? 'var(--accent-light)' : 'var(--bg-secondary)',
                                                        color: invite.role === 'admin' ? 'var(--accent)' : 'var(--text-secondary)',
