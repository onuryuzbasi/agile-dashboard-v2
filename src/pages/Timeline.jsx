...
export default function Timeline() {
    ...
    const filteredIssues = useMemo(() => {
        return activeIssues.filter(issue => {
            ...
            if (filterEpics.size > 0) {
                ...
                if (!isEpic && !filterEpics.has(issue.parentId)) return false
            }
            return true
        })
    ...
    const groupedData = useMemo(() => {
        ...
        if (groupBy === 'epic') {
            ...
            // Add child issues to their epics (epics are linked via parentId)
            filteredIssues.forEach(issue => {
                if (issue.type === 'epic') return
                // Issues link to epics via parentId (not epicId)
                const epicId = issue.parentId || 'no-epic'
    ...
    const handleBarMouseDown = useCallback((e, issue, handleType) => {
        e.preventDefault()
        e.stopPropagation()
        // Use calculated dates for epics, regular dates for issues
        const startDate = issue.calculatedStartDate || issue.startDate
        const endDate = issue.calculatedDueDate || issue.dueDate
        setDragState({
            issue,
    ...
    const handleBarDrag = useCallback((e) => {
        ...
        try {
            originalStart = dragState.originalStart ? new Date(dragState.originalStart) : null
            originalEnd = dragState.originalEnd ? new Date(dragState.originalEnd) : null
            ...
            if (!originalEnd || isNaN(originalEnd.getTime())) {
                originalEnd = addDays(originalStart, 1)
            }
        } catch (err) {
        ...
        if (dragState.handleType === 'move') {
            // Move entire bar - both dates shift by same amount
    ...
    const handleBarDragEnd = useCallback(async () => {
        ...
        // Mark that we just finished a drag - this prevents click handler from opening modal
        ...
        return (
            ...
            >
                ...
                <div
                    ...
                    onClick={(e) => {
                        // Only open modal if this wasn't a drag operation
                        if (!dragState?.hasMoved && !wasDraggingRef.current) {
