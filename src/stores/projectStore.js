    ...
    persist(
        (set, get) => ({
            ...
            addComment: async (issueId, text, userId = null) => {
                ...
                const newComments = [...(issue.comments || []), commentEntry]

                // Optimistic update
                set({
                    issues: state.issues.map(i =>
                        i.id === issueId
                            ? { ...i, comments: newComments, updatedAt: new Date().toISOString() }
                            : i
                    )
                })
            ...
            getAvailableCardFields: () => {
                ...
                const fields = [
                    ...
                    { key: 'parent', label: 'Parent Epic', category: 'standard' }
                ]
        ...
        }),
        {
            ...
            migrate: (persistedState, version) => {
                // Remove theme from persisted state so it always defaults to 'light'
            ...
            merge: (persistedState, currentState) => {
                // Merge persisted state but always force theme to 'light'
